...
import { symptomAssessmentSchema } from "@/schemas/symptomSchemas";
...
export function SymptomsPage() {
  ...
  const handleAssess = () => {
    const parsed = symptomAssessmentSchema.safeParse({ selected, severity });
    if (!parsed.success) {
      const msg = parsed.error.issues[0]?.message ?? "Please complete the form";
      setFormError(msg);
      toastError(msg);
      return;
    }
    ...
    <>
      <div className="max-w-3xl">
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm text-red-700 mt-0.5">
                Heart palpitations with high severity may require immediate medical attention.
                Please contact your doctor or call emergency services.
              </p>
              <button
                type="button"
        ...
        <Card className="mb-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3" role="group" aria-label="Symptoms">
            {symptoms.map((s) => {
              const Icon = s.icon;
        ...
        <Card className="mb-5">
          ...
          >
            Severity Level:{" "}
            <span style={{ color: severity <= 2 ? GREEN : severity <= 3 ? AMBER : RED }}>
              {severityLabels[severity]}
            </span>
          </h3>
          <label className="sr-only" htmlFor="severity-range">
            Severity from 1 mild to 5 critical
          </label>
          <input
            id="severity-range"
            type="range"
            ...
            aria-valuetext={severityLabels[severity]}
          />
        ...
        <Btn size="lg" type="button" onClick={handleAssess}>
          <Stethoscope className="w-5 h-5" aria-hidden="true" /> Get AI Assessment
        </Btn>

        {assessed && (
          <Card
            className="mt-5 border-l-4"
            style={{ borderLeftColor: severity >= 4 ? RED : severity >= 3 ? AMBER : GREEN }}
          >
            <div className="flex items-start gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Based on your selected symptoms and severity
                </p>
              </div>
            </div>
            <div className="space-y-3 text-sm text-muted-foreground">
              <p>
                ...
                at severity level {severity}, here is my assessment:
              </p>
              <p>
                These symptoms are{" "}
                <strong className="text-foreground">
                  {severity <= 2
                    ...
                    : severity <= 3
                      ...
                      : "concerning and require attention"}
                </strong>{" "}
                for someone 9 months post-thyroidectomy. Fatigue and dry mouth are commonly
                experienced during the hormonal adjustment period.
              </p>
